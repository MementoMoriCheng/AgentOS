# SessionLoop 单栈切换 — 结项归档（POSTMORTEM）

> **归档日期**：2026-07-09
> **状态**：Waves 0.5–5 ✅ 代码完成；`develop` @ `c232cce`；tag `pre-session-loop-cutover`
> **活跃 SSOT**：实施期文档见同目录 [`SESSION-LOOP-SINGLE-STACK-CUTOVER.md`](./SESSION-LOOP-SINGLE-STACK-CUTOVER.md)（维护约定已结项）
> **策略**：废弃 1.0 双循环，SessionLoop 作为唯一会话调度内核，**无 feature flag、无降级路径**

## 结项摘要

| Wave | 结果 | 关键 commit / 日期 |
|------|------|-------------------|
| 0.5 SDK 子包拆分 | ✅ | 2026-07-07 |
| 1 SessionLoop 内核 | ✅ | `feature/wave-1-session-loop` |
| 2 Port 收敛 + 影子验证 | ✅ | `feature/wave-2-port-completion` |
| 3 上层原子切换 | ✅ | `641d3c3` 2026-07-09 |
| 4 1.0 物理删除 | ✅ | `73c36f3` / `641d3c3` 2026-07-09 |
| 5 bus / LongRun / 多副本前置 | ✅ | `91c722f` 2026-07-09 |

### 未完成（运维/验证，非代码阻塞）

- Jenkins 部署 + `gnex-core-v2.dev.local` hosts（用户明确跳过）
- 远程 dev.local E2E（`ENOTFOUND gnex-core-v2.dev.local`）
- 灰度 tenant 1 周观察
...
| 项 | 结果 |
|----|------|
| `gnex-dev-assembler` 启动 | ✅ `:8080/actuator/health` 200（Spring 循环依赖 `@Lazy` 修复） |
| 单测 `gnex-agent-sdk` / `gnex-agent-service` / `gnex-bus-service` | ✅ 全绿 |
| Integration E2E（`:5174`） | ✅ 9/9（`新对话` 按钮文案修复） |
| Local API d4-runtime | ✅ 5 passed / 1 skipped（熔断用例 skip） |
| Redis `SessionStatePort` | ✅ `RedisSessionStatePort`（`@ConditionalOnBean(RedisAdapter)`）+ InMemory fallback |
| `osb_tracked_provider` | ✅ V77 迁入主库 `gnex-persistence-common` |
| 引擎 `task_lease` | ✅ V78 schema + `TaskLeaseService` / `AgentScheduler` 接线 + `TaskLeaseWatchdog` |
| Integration E2E operations | ✅ `operations.spec.ts` 重写（UTF-8，对齐 glossary） |
| Always-On → SessionLoop | ✅ `SessionLoopReActLoop` 为 `@Primary` `ReActLoopPort`；`SessionStatePort` 注册 `always-on:{taskId}` |

### CUTOVER Wave 5 交付物

- `BusTaskDispatchPort` + `AgentInboxLoopRegistry` + `MessageBusConsumer` 接线
- `BUDGET_EXHAUSTED` / `LongRunSliceCoordinator` / `run.continue` / `RunLeaseWatchdog`
- `SessionLoopRecoveryService` + 时序 SSOT [`SESSION-LONG-RUN-TIMING.md`](./v2.0/Agent核心/SESSION-LONG-RUN-TIMING.md)

### 引擎 Wave 5 交付物（`EXECUTION-ENGINE-SPEC` §13.1）

> 与上表 **CUTOVER Wave 5** 同名不同维；结项日期同为 2026-07-09。

| 项 | 结果 |
|----|------|
| `V78__task_lease` + `TaskLeaseService` / `TaskLeaseWatchdog` | ✅ `d70c48c` |
| `ReActCheckpointService` checkpoint lease | ✅ `ReActCheckpointLeaseTest` |
| `SseEvent.BUDGET_EXCEEDED` + `AgentBudgetService` | ✅ `AgentBudgetServiceTest` |
| SessionLoop 全优先级 inbound 清空（替代 MessageQueue followUp 漏清） | ✅ `PriorityInboundQueueTest` / `SessionLoopChatConcurrencyTest` |
| `InspectorChain` fail-safe DENY | ✅ `InspectorChainTest` |

**未纳入**：`ActiveRunRegistry` DB 落库（仍内存；见 spec §13.1 脚注）。

---

# SessionLoop 单栈切换实施计划（归档副本）

> **状态**：Waves 0.5–5 ✅（2026-07-09 结项归档）
> **策略**：废弃 1.0 双循环，SessionLoop 作为唯一会话调度内核，**无 feature flag、无降级路径**
...
**Wave 3-5 之间的 72h 任务业务连续性**：Always-On 经 `SessionLoopReActLoop` + `SessionStatePort`（`always-on:{taskId}`）调度。
...
| Wave 3-5 间 72h 任务支持 | Always-On 经 SessionLoopReActLoop + SessionStatePort |
...
- 6 大不变量测试用例更新 → 更新 §5
- DB schema 变更 → 更新 §6
- Wave 4 完成后本文档归档为 `SESSION-LOOP-SINGLE-STACK-CUTOVER-POSTMORTEM.md`
