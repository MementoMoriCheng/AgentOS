...
class Pipeline:
    ...
    async def call(self, sess: Session, sandbox_id: str, tool_name: str,
                   params: Dict[str, Any], run_id: str = "") -> PipelineResponse:
        # 步骤 1:查找工具。未知工具 = 拒绝。
        tool, ok = self.registry.get(tool_name)
        if not ok:
            await self.bus.publish(Event(type="tool.denied", session_id=sess.id, run_id=run_id, tool=tool_name, params=params))
        ...
        try:
            ...
            await self.bus.publish(Event(type="quota.exceeded", session_id=sess.id, run_id=run_id, tool=tool_name, params=params))
        ...
        if "error" in exec_result:
            await self.bus.publish(Event(type="tool.errored", session_id=sess.id, run_id=run_id, tool=tool_name, params=params))
        ...
        await self.bus.publish(Event(
            type="tool.called", session_id=sess.id, run_id=run_id, tool=tool_name,
